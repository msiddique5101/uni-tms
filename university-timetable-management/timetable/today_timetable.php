...
$current_day = date('l'); // e.g., Saturday
...
if ($user_role === 'admin') {
    $sql = "SELECT tt.*, 
            ...
            WHERE tt.day_of_week = '$current_day'";
    ...
    $sql = "SELECT tt.*, 
            ...
            JOIN subjects s ON tt.subject_id = s.subject_id
            JOIN classrooms r ON tt.room_id = r.room_id
            WHERE tt.teacher_id = $user_id AND tt.day_of_week = '$current_day'";
    ...
    if ($class_result && $class_result->num_rows > 0) {
        ...
        $sql = "SELECT tt.*, 
                ...
                JOIN subjects s ON tt.subject_id = s.subject_id
                JOIN classrooms r ON tt.room_id = r.room_id
                WHERE tt.class_id = $class_id AND tt.day_of_week = '$current_day'";
    } else {
...
<div class="container mt-4">
    <h4 class="mb-3"><i class="fas fa-calendar-alt"></i> Today's Timetable (<?= htmlspecialchars($current_day) ?>)</h4>
                ...
                <table class="table table-bordered table-hover align-middle">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th>Start Time</th>
                            <th>End Time</th>
                            <?php if ($user_role === 'admin' || $user_role === 'student') echo "<th>Teacher</th>"; ?>
                            <?php if ($user_role === 'admin' || $user_role === 'teacher') echo "<th>Class</th>"; ?>
                            <th>Subject</th>
                            <th>Room No</th>
                    ...
                    <tbody>
                        ...
                        if ($result && $result->num_rows > 0) {
                            ...
                            $colspan = 4;
                            if ($user_role === 'admin') $colspan += 4;
                            elseif ($user_role === 'teacher') $colspan += 2;
                            elseif ($user_role === 'student') $colspan += 2;

                            echo "<tr><td colspan='$colspan' class='text-center'>No timetable records found for today.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
    ...
    <div class="mt-4">
        <a href="view_timetable.php" class="btn btn-outline-primary">View Full Timetable</a>
    </div>
</div>
